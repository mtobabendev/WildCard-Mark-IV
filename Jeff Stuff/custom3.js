...
const productDiv = document.querySelector("#custom");
    ...
    const products = [
        {
            name: "1",
            description: "Culinary Website",
            image: "assets/images/culi.png",
            quantity: 1,
        },
        {
            name: "2",
            description: " Ecommerce Website",
            image: "assets/images/Capture11.PNG",
            quantity: 1,
        },
        {
            name: "3",
...
for (let i = 0; i < productsSlice.length; i++) {
        ...
        // image on click event listener
        function displayRemove2(text, image, product) {
          image.addEventListener('click', () => {
            removeItem(product);
            text.textContent = 'Add To Checkout🛒';
              displayAdd2(text, image, product);
          });
        }

        function displayAdd2(text, image, product) {
          image.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove2(text, image, product);
          })
        }
        //button on click event listener
        function displayRemove4(text, button, product) {
          button.addEventListener('click', () => {
            removeItem(product);
            text.textContent = 'Add To Checkout🛒';
              displayAdd4(text, button, product);
          });
        }

        function displayAdd4(text, button, product) {
          button.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove4(text, button, product);
          })
        }
        ...
        displayAdd2(addToCart, image, productsSlice[i]);
        displayAdd4(addToCart, addToCart, productsSlice[i]);
...
for (let i = 0; i < productsApp.length; i++) {
        ...
        //image eventlistener function for changing the inner html of the button and allow an on click add and remove function from the checkout
        function displayRemove1(text, image, product) {
          image.addEventListener('click', () => {
            removeItem(product);
              text.textContent = 'Add To Checkout🛒';
              displayAdd1(text, image, product);
          });
        }
        //image eventlistener function for changing the inner html of the button and allow an on click add and remove function from the checkout
        function displayAdd1(text, image, product) {
          image.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove1(text, image, product);
          })
        }
        //Button eventlistener function for changing the inner html of the button and allow an on click add and remove function from the checkout
        function displayRemove3(text, button, product) {
          button.addEventListener('click', () => {
            removeItem(product);
            text.textContent = 'Add To Checkout🛒';
            displayAdd3(text, button, product);
          });
        }
        
        //Button eventlistener function for changing the inner html of the button and allow an on click add and remove function from the checkout
        function displayAdd3(text, button, product) {
          button.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove3(text, button, product);
          })
        }
        ...
        image.classList.add("images2");
        displayAdd1(addToCart, image, productsApp[i]);
        displayAdd3(addToCart, addToCart, productsApp[i]);
...
var productslogo = products.slice(6, 10);
for (let i = 0; i < productslogo.length; i++) {
        ...
        // function displayRemove(text, image, product) {
        //   image.addEventListener('click', () => {
        //     removeItem(product);
        //     text.textContent = "Removed";
        //     setTimeout(() => {
        //       text.textContent = 'Add To Checkout🛒';
        //       displayAdd(text, image, product);
        //     }, 2000);
        //   });
        // }
        function displayRemove(text, image, product) {
        ...
        function displayAdd(text, image, product) {
          image.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove(text, image, product);
          })
        }

        function displayRemove5(text, button, product) {
          button.addEventListener('click', () => {
            removeItem(product);
            text.textContent = 'Add To Checkout🛒';
            displayAdd5(text, button, product);
          });
        }
        function displayAdd5(text, button, product) {
          button.addEventListener('click', () => {
            console.log(product);
            addItem(product);
            text.textContent = "Click again to Remove🛒";
            displayRemove5(text, button, product);
          })
        }
        ...
        image.classList.add("images2");
        displayAdd(addToCart, image, productslogo[i]);
        displayAdd5(addToCart, addToCart, productslogo[i]);
        addToCart.classList.add("buttonCart");
        addToCart.classList.add("addToCart2");
        addToCart.textContent = "Add To Checkout🛒";
    ...
    function fetchForm() {
      ...
      var y = document.forms["myForm"]["aname"].value;
      var z = document.forms["myForm"]["bname"].value;
        if (x == "") {
          var a = document.getElementById("snackbar1");
        ...
        if(y == '') {
          var b = document.getElementById("snackbar2");
        ...
        if(z == '') {
          var c = document.getElementById("snackbar3");
        ...
        const orderUsername = document.getElementById('Login').innerHTML;
        ...
        fetch('http://localhost:5000/form', {
          ...
          body: JSON.stringify({ nameForm: nameForm, emailForm: emailForm, phoneForm: phoneForm, commentForm: commentForm, orderUsername: orderUsername  })
        })
    ...
    function fetchregister() {
      ...
      .then(data => {
        if(data.message === 'Register Successful') {
         ...
         // Once it's parsed as a json, we just log it to the console
...
window.onclick = function(event) {
  ...
  function fetchlogin (e) {
    ...
    function myFunction1() {
      ...
      var i = document.getElementById("snackbar4");
    
      // Add the "show" class to DIV
      i.className = "show";
    
      // After 3 seconds, remove the show class from DIV
      setTimeout(function(){ i.className = i.className.replace("show", ""); }, 3000);
    }
